...
export default function NextPeriod({ persons }: Props) {
  ...
  if (!period) {
    return (
      <div className="rounded-3xl border border-white/10 bg-white/5 p-5 shadow-2xl backdrop-blur-md">
        ...
        <p className="text-sm text-gray-400">No more periods today.</p>
      </div>
    );
  }
  ...
  return (
    <section className="overflow-hidden rounded-3xl border border-white/10 bg-white/5 shadow-2xl backdrop-blur-md">
      <div className="border-b border-white/10 bg-white/5 px-5 py-4 text-white">
        <div className="flex items-center justify-between">
          <h2 className="text-sm font-bold uppercase tracking-[0.14em] text-cyan-400">
            Next Period
          </h2>
        ...
        <div className="rounded-2xl border border-rose-500/20 bg-rose-500/5 p-4">
          ...
          <div className="space-y-2">
            {busy.length ? (
              ...
              <p className="text-sm text-rose-400/80">No classes scheduled.</p>
            )}
          </div>
        </div>
      </div>
